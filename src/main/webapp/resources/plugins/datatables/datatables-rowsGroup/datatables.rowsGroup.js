...
var RowsGroup = function ( dt, columnsForGrouping )
{
	...
	this.order = [];
	this.page = 0;	/* 최한나 수정 : page 유지기능 추가 */
	...
	dt.on('page.dt', function ( e, settings) {
		self.page = self.table.page.info().page;	/* 최한나 수정 : page 유지기능 추가 */
		self.mergeCellsNeeded = true;
	});
...
RowsGroup.prototype = {
	...
	_updateOrderAndDraw: function()
	{
		...
		/* 최한나 수정 : page 유지기능 추가 */
		// this.table.draw();
		this.table.page( this.page ).draw('page');

		for (idx = 0; idx < this.table.order().length; idx++) {
			if (this.table.settings()[0].aoColumns[this.table.order()[idx][0]].orderable == false) {
				this.table.settings()[0].aoColumns[this.table.order()[idx][0]].nTh.className = "sorting_disabled";
			}
		}
	},
};
