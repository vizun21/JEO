<script>
	$(function () {
		var filePath = BasePath + "/resources/files/docs/supplementary_budget.hwp";
		...
		PutFieldText("[예산구분]", "${budg_type}" == "0" ? "본" : "${budg_type}차추경 ");
		PutFieldText("[이전예산구분]", "${budg_type}" == "0" ? "전년도 " : ("${budg_type}" == "1" ? "본" : "${compare_budg_type}차추경 "));
		...
		$.ajax({
			...
			, data: JSON.stringify({
				budg_year: "${budg_year}"
				, budg_type: "${budg_type}"
				, compare_budg_year: "${compare_budg_year}"
				, compare_budg_type: "${compare_budg_type}"
				, io_type: "${io_type}"
			})
			, beforeSend: function () {
				$(".overlay").show();
